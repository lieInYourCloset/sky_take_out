...
//        List<Orders> orders = ordersMapper.findTimeoutPendingOrders();
...
//        log.info("定时处理派送中的订单");
//        List<Orders> orders = ordersMapper.findDeliveringOrders();
//        if (!CollectionUtils.isEmpty(orders)) {
//            for (Orders order : orders) {
...
//        log.info("测试WebSocket，定时发送信息");
//        webSocketServer.sendToAllClient("当前时间：" + LocalDateTime.now());
//    }
}
